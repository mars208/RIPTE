 ...
 * codelength = length of the BCH code
 ...
 * bb[] = coefficients of redundancy polynomial x^(codelength-k) i(x) modulo g(x)
...
int             m, n, codelength, k, t, d;
int             p[11];
int             alpha_to[1024], index_of[500], g[500];
 ...
 */
{
  ...
  /*
  printf("\nFirst, enter a value of m such that the code codelength is\n");
  printf("2**(m-1) - 1 < codelength <= 2**m - 1\n\n");
  do {
...
/*
  do  {
    printf("Enter code codelength (%d < codelength <= %d): ", ninf, n);
    scanf("%d", &codelength);
  } while ( !((codelength <= n)&&(codelength>ninf)) );
  */
  codelength = 1020;
}
 ...
 */
{
  ...
  int             cycle[440][19], size[1024], min[1024], zeros[1024];
 ...
 * polynomial b(x) is the remainder after dividing x^(codelength-k)*data(x)
 ...
 */
{
  ...
  for (i = 0; i < codelength - k; i++)
    bb[i] = 0;
  for (i = k - 1; i >= 0; i--) {
    feedback = data[i] ^ bb[codelength - k - 1];
    if (feedback != 0) {
      for (j = codelength - k - 1; j > 0; j--)
	if (g[j] != 0)
 ...
 */
{
  ...
  int             elp[440][20], d[500], l[500], u_lu[500], s[500];
